      ...
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0 text-dark">Bussiness Report
            <small class="text-sm">ANNUAL TRANSACTION SUMMARY: YEAR <?php echo date('Y'); ?></small></h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
      ...
      $month = date("F", mktime(0, 0, 0, $b, 10)) ; ?>
               ...
               if($sta->yearSales($yy,$pid)>0){
                    ...
                    <tr class="odd gradeX" >
                        <td class="center"><?php echo $e ?></td>
                         ...
                         <?php $a=1; while($a<=12){$b=$a++; $mon = ($b<10) ? '0'.$b : $b; $month = date('Y').$mon; ?>
                          <td <?php if($mm==$b){ ?> bgcolor="#99FF33" <?php  } ?> ><?php echo number_format($sta->monthSales($month,$pid));  ?></td>
                        <?php } ?>
                         
                         <td><?php echo number_format($sta->yearSales($yy,$pid));   ?></td>
                         <td  class="center noprint"><a href="#" onclick="BrWindow('itemdetails.php?transactionIndex=<?php echo $row->id; ?>&Item=<?php echo $row->item; ?>','','width=800,height=600')" > Details </a></td>
                 ...
                 <tr>
                        ...
                        <?php $a=1; while($a<=12){$b=$a++; $mon = ($b<10) ? '0'.$b : $b; $month = date('Y').$mon; ?>
                          <th  <?php if($mm==$b){ ?> bgcolor="#99FF33" <?php  } ?>><?php echo number_format($sta->monthSalesTotal($month)); ?></th>
                        <?php } ?>
